<script lang="ts">
	...
	import { setValue, setKey, addProperty, removeNode, type TreeNode } from '@visual-json/core';
	import {
		getDisplayKey,
		getResolvedSchema,
	...
	function focusValueInput() {
		if (valueInputRef && 'focus' in valueInputRef && typeof valueInputRef.focus === 'function') {
			valueInputRef.focus();
		}
	}
	...
	const parentIsObject = $derived(
		node.parentId ? studioState.tree.nodesById.get(node.parentId)?.type === 'object' : false
	);

	const propSchema = $derived(getResolvedSchema(ctx.schema, ctx.rootSchema, node.path));
	const isRequired = $derived(checkRequiredFn(node, ctx.schema, ctx.rootSchema));
	...
	function getBorderTopColor() {
		if (isDragTarget && ctx.dragState.dropPosition === 'before') return 'var(--vj-accent, #007acc)';
		return 'transparent';
	}

	function getBorderBottomColor() {
		if (isDragTarget && ctx.dragState.dropPosition === 'after') return 'var(--vj-accent, #007acc)';
		return 'transparent';
	}
	...
	function handleAddChild() {
		const key =
			node.type === 'array' ? String(node.children.length) : `newKey${node.children.length}`;
...
{#if isContainer}
	<div>
		<div
			...
			ondrop={(e) => { e.preventDefault(); ctx.onDrop(); }}
		>
			<button
				style="
					background: none;
				...
				"
				onclick={(e) => { e.stopPropagation(); ctx.onToggleCollapse(node.id); }}
			>
				&#9654;
			</button>
...
{:else}
	...
	>
		...
		{#if isRequired && !isSelected}
			<span style="color: var(--vj-error, #f48771); font-size: 10px; font-family: var(--vj-font, monospace);">
				*
			</span>
		{/if}
