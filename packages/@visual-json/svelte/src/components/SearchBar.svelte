		...
		{#if studioState.searchQuery}
			<span
				style="
					font-size: 11px;
					line-height: 1;
					color: {studioState.searchMatches.length > 0 ? 'var(--vj-text-muted, #999999)' : 'var(--vj-error, #f48771)'};
					font-family: var(--vj-font, monospace);
					white-space: nowrap;
				"
			>
			...
			<button
				...
				style="
					background: none;
					border: none;
					color: {studioState.searchMatches.length > 0 ? 'var(--vj-text, #cccccc)' : 'var(--vj-text-dimmer, #555555)'};
