<script lang="ts">
	...
	const metrics = $derived.by(() => {
		...
		for (const node of visibleNodes) {
			const keyText = node.parentId === null ? '/' : getDisplayKey(node, studioState.tree);
	...
	function handleKeyDown(e: KeyboardEvent) {
		...
		let currentIndex = visibleNodes.findIndex((n) => n.id === studioState.focusedNodeId);
		...
		switch (e.key) {
			...
			case 'ArrowLeft': {
				...
				if (isNodeContainer && !collapsedIds.has(current.id)) {
					...
					const parentInVisible = visibleNodes.find((n) => n.id === current.parentId);
					if (parentInVisible) {
	...
	const formCtx: FormViewContext = {
		...
		get maxDepth() { return metrics.maxDepth; },
