<script lang="ts">
	...
	function isActiveMatch() {
		return (
			...
			studioState.searchMatches[studioState.searchMatchIndex]?.nodeId === node.id
		);
	}
	...
	function getBorderTopColor() {
		if (dragState.dropTargetNodeId === node.id && dragState.dropPosition === 'before') {
	...
	function getBorderBottomColor() {
		if (dragState.dropTargetNodeId === node.id && dragState.dropPosition === 'after') {
			return 'var(--vj-accent, #007acc)';
		}
		return 'transparent';
	}
