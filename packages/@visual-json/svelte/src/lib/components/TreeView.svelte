<script lang="ts">
	...
	import { duplicateNode, changeType, toJson, type NodeType, type TreeNode } from '@visual-json/core';
	import { deleteSelectedNodes, computeSelectAllIds } from '../selection-utils.js';
	...
	let { class: className, showValues = true, showCounts = false }: Props = $props();
	...
	const visibleNodes = $derived(
		getVisibleNodes(studioState.tree.root, (id) => studioState.expandedNodeIds.has(id))
	);

	const { dragState, handleDragStart, handleDragOver, handleDragEnd, handleDrop } = useDragDrop(
		() => visibleNodes,
	...
	function handleKeyDown(e: KeyboardEvent) {
		const currentIndex = visibleNodes.findIndex((n) => n.id === studioState.focusedNodeId);
		...
		switch (e.key) {
			...
			case 'ArrowLeft': {
				...
				if (!current) break;
				const isNodeContainer = current.type === 'object' || current.type === 'array';
			...
			case 'a': {
				if (e.metaKey || e.ctrlKey) {
					...
					if (ids) {
						actions.setSelection(studioState.focusedNodeId, ids, studioState.focusedNodeId);
					}
				}
				break;
			}
