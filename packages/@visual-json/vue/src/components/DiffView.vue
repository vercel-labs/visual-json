...
<template>
  ...
  >
    ...
    >
      ...
      <span v-if="added > 0" :style="{ color: '#4ec94e' }"
        >+{{ added }} added</span
      >
      <span v-if="removed > 0" :style="{ color: '#f48771' }"
        >-{{ removed }} removed</span
      >
      <span v-if="changed > 0" :style="{ color: '#dcdcaa' }"
        >~{{ changed }} modified</span
      >
    </div>
    <div :style="{ flex: 1, overflow: 'auto' }">
      ...
      >
        ...
        <span
          :style="{ flex: 1, display: 'flex', gap: '8px', overflow: 'hidden' }"
        >
          <template v-if="entry.type === 'changed'">
            <span :style="{ color: '#f48771', textDecoration: 'line-through' }">
              {{ formatValue(entry.oldValue) }}
            </span>
            <span :style="{ color: 'var(--vj-text-dim, #666666)' }"
              >&rarr;</span
            >
