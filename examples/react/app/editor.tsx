...
export function Editor({
  ...
  useEffect(() => {
    const currentSample = samples.find((s) => s.filename === filename);
    if (currentSample?.schema !== undefined) return;
    let cancelled = false;
      ...
      <Dialog open={pasteDialogOpen} onOpenChange={setPasteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <textarea
            ...
            spellCheck={false}
            className="min-h-50 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm font-mono resize-none outline-none"
          />
          <DialogFooter>
            <DialogClose asChild>
